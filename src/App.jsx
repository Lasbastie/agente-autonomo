...
function Btn({ children, onClick, disabled, variant = "primary", style: s = {} }) {
  ...
  const variants = {
    primary: { background: "#a78bfa", color: "#0a0a18" },
    secondary: { background: "transparent", color: "#6a6a8a", border: "1px solid #2a2a3e" },
  };
...
function TelaOnboarding({ usuario, onConcluir }) {
  ...
  const steps = [
    {
      title: "Seu negócio", valid: f.negocio && f.nicho,
      content: <>
        <Input label="Nome do negócio" value={f.negocio} onChange={v => u("negocio", v)} placeholder="Ex: Studio Fitness da Ana" />
        <Input label="Nicho / área" value={f.nicho} onChange={v => u("nicho", v)} placeholder="Ex: Saúde feminina" />
        <Input label="Público-alvo" value={f.publico} onChange={v => u("publico", v)} placeholder="Ex: Mulheres 30-45 anos" />
        <Input label="Objetivo principal" value={f.objetivo} onChange={v => u("objetivo", v)} placeholder="Ex: Lançar meu curso" />
      </>
    },
    {
      title: "Tom e módulos", valid: f.tom.length > 0 && f.modulos.length > 0,
      content: <>
        <div style={{ marginBottom: 20 }}>
          <label style={{ fontSize: 12, color: "#6a6a8a", display: "block", marginBottom: 10 }}>Tom de voz</label>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            {TONS.map(t => <Tag key={t} label={t} selected={f.tom.includes(t)} onClick={() => tog("tom", t)} />)}
          </div>
        </div>
        <div>
          <label style={{ fontSize: 12, color: "#6a6a8a", display: "block", marginBottom: 10 }}>Módulos</label>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            {MODULOS.map(m => <Tag key={m} label={m} selected={f.modulos.includes(m)} onClick={() => tog("modulos", m)} />)}
          </div>
        </div>
      </>
    },
  ...
  return (
    <div style={{ maxWidth: 500, margin: "0 auto", padding: "32px 24px" }}>
      <div style={{ display: "flex", gap: 6, marginBottom: 28 }}>
        {steps.map((_, i) => <div key={i} style={{ flex: 1, height: 3, borderRadius: 2, background: i <= step ? "#a78bfa" : "#1e1e3a" }} />)}
      </div>
      ...
      <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
        ...
        {step < steps.length - 1
          ? <Btn onClick={() => setStep(s => s + 1)} disabled={!steps[step].valid} style={{ flex: 1 }}>Continuar →</Btn>
          : <Btn onClick={concluir} disabled={loading || !steps[step].valid} style={{ flex: 1 }}>{loading ? "Configurando..." : "✦ Ativar Agente"}</Btn>}
      </div>
    </div>
  );
}
    ...
    <div style={{ maxWidth: 580, margin: "0 auto", padding: "20px" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            <div style={{ fontSize: 14, fontWeight: 600, color: "#f0f0ff" }}>Olá, {usuario.nome}!</div>
            <div style={{ fontSize: 11, color: "#6a6a8a" }}>{perfil?.negocio} · {redesConectadas} rede{redesConectadas !== 1 ? "s" : ""} conectada{redesConectadas !== 1 ? "s" : ""}</div>
          </div>
        </div>
      ...
      {aba === "agente" && (
        <div>
          <div style={{ height: 360, overflowY: "auto", marginBottom: 14, display: "flex", flexDirection: "column", gap: 10 }}>
            {msgs.length === 0 && (
              <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", color: "#4a4a6a", gap: 6 }}>
                <div style={{ fontSize: 28, color: "#a78bfa" }}>◈</div>
                <div style={{ fontSize: 14 }}>Agente pronto!</div>
                <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 16, width: "100%" }}>
                  {["Crie uma legenda para Instagram", "Escreva um follow-up de WhatsApp", "Monte meu plano da semana"].map(s => (
                    <button key={s} onClick={() => setInput(s)} style={{ padding: "9px 14px", borderRadius: 10, border: "1px solid #2a2a3e", background: "#0d0d1a", color: "#8a8ab0", fontSize: 12, cursor: "pointer", textAlign: "left" }}>
            ...
            {msgs.map((m, i) => (
              <div key={i} style={{ display: "flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start" }}>
                <div style={{ maxWidth: "80%", padding: "10px 14px", borderRadius: m.role === "user" ? "16px 16px 4px 16px" : "16px 16px 16px 4px", background: m.role === "user" ? "#a78bfa22" : "#1a1a2e", border: `1px solid ${m.role === "user" ? "#a78bfa44" : "#2a2a3e"}`, fontSize: 13, color: "#e2e8f0", lineHeight: 1.6, whiteSpace: "pre-wrap" }}>{m.content}</div>
              </div>
            ))}
      ...
      {aba === "redes" && (
        <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {REDES.map(r => (
              <div key={r.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", borderRadius: 12, border: `1px solid ${conexoes[r.id] ? r.color + "44" : "#1e1e3a"}`, background: conexoes[r.id] ? r.color + "11" : "#0d0d1a" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <span style={{ fontSize: 18 }}>{r.icon}</span>
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 500, color: conexoes[r.id] ? "#e2e8f0" : "#8a8ab0" }}>{r.label}</div>
                    {conexoes[r.id] && <div style={{ fontSize: 11, color: r.color }}>● conectado</div>}
                  </div>
                </div>
                <button onClick={() => setConexoes(p => ({ ...p, [r.id]: !p[r.id] }))} style={{ padding: "6px 14px", borderRadius: 8, fontSize: 12, cursor: "pointer", border: `1px solid ${conexoes[r.id] ? r.color + "66" : "#2a2a3e"}`, background: conexoes[r.id] ? r.color + "22" : "#12122a", color: conexoes[r.id] ? r.color : "#6a6a8a", fontFamily: "inherit" }}>
                  {conexoes[r.id] ? "✓ Conectado" : "Conectar"}
                </button>
              </div>
            ))}
          </div>
      ...
      {aba === "perfil" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <div style={{ background: "#0d0d1a", borderRadius: 12, border: "1px solid #1e1e3a", padding: 16 }}>
            ...
            {[["Nome", usuario.nome], ["E-mail", usuario.email], ["Negócio", perfil?.negocio], ["Nicho", perfil?.nicho], ["Público", perfil?.publico], ["Objetivo", perfil?.objetivo], ["Tom", perfil?.tom?.join(", ")], ["Módulos", perfil?.modulos?.join(", ")]].filter(([, v]) => v).map(([l, v]) => (
              <div key={l} style={{ display: "flex", gap: 12, padding: "8px 0", borderBottom: "1px solid #1a1a2e" }}>
                <span style={{ fontSize: 12, color: "#4a4a6a", minWidth: 80 }}>{l}</span>
                <span style={{ fontSize: 13, color: "#c2c2e2" }}>{v}</span>
              </div>
            ))}
          </div>
          <div style={{ background: "#0d0d1a", borderRadius: 12, border: "1px solid #1e1e3a", padding: 16 }}>
